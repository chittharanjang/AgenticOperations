...
/**
 * Extract a short, human-readable message from a Kognitos exception description.
 * The description is typically a long traceback; this pulls the root-cause message.
 */
export function extractExceptionMessage(description: string): string {
  const allErrors = [
    ...description.matchAll(/(?:Error|Exception):\s*([^"\\\n,]{5,150})/g),
  ].map((m) => m[1].trim());

  const meaningful = allErrors.filter(
    (m) => !m.includes("http://") && !m.includes("https://"),
  );
  if (meaningful.length > 0) return meaningful[0];
  if (allErrors.length > 0) return allErrors[allErrors.length - 1];

  const msgMatch = description.match(/"message"\s*:\s*"([^"]{5,150})"/);
  if (msgMatch) return msgMatch[1];

  return "Exception during screening";
}
...
export async function pollRun(
  ...
  while (Date.now() < deadline) {
    ...
    if (data.state?.awaiting_guidance) {
      const desc = data.state.awaiting_guidance.description;
      return {
